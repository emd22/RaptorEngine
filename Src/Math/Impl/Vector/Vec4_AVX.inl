...
FX_FORCE_INLINE Vec4f::Vec4f(float32 scalar) { mIntrin = _mm_set1_ps(scalar); }


FX_FORCE_INLINE float32 Vec4f::LengthSquared() const { return SSE::LengthSquared(mIntrin); }

FX_FORCE_INLINE void Vec4f::Set(float32 x, float32 y, float32 z, float32 w) { mIntrin = _mm_setr_ps(x, y, z, w); }
FX_FORCE_INLINE void Vec4f::Set(float32 scalar) { mIntrin = _mm_set1_ps(scalar); }
