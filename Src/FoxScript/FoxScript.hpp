...
public:
	...
	FoxValue CallProc(const FoxSymbol* sym, const SizedArray<FoxValue>& args);
	...
	FX_FORCE_INLINE const FoxSymbol* GetSymbol(const String& name) const
	{
		return Vm.GetSymbol(HashStr32(name.CStr()));
	}
	FX_FORCE_INLINE const FoxSymbol* GetSymbol(const Hash32 name_hash) const { return Vm.GetSymbol(name_hash); };
